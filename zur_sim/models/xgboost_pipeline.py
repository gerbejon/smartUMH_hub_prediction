"""
XGBoost (Recursive Multi-Step) Forecasting Pipeline
====================================================
Gradient-boosted-tree pipeline that mirrors the structure of
sarima_pipeline.py / ets_pipeline.py so it plugs into calculate_all.py
unchanged.
...
# ─────────────────────────────────────────────
# FEATURE DEFINITION  (XGBoost-specific)
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# FIT + FORECAST  (XGBoost-specific)
# ─────────────────────────────────────────────

def fit_xgboost(series_train: pd.Series, params: dict):
    """
    Build the supervised training frame from the training series and fit
    an XGBRegressor. Returns (model, train_frame).
    """
    ...
    print(f"\n[Model] XGBoost fit on {len(train_df)} supervised rows "
...
def xgboost_pipeline(
    ...
    """
    End-to-end XGBoost recursive pipeline with train/test split.
    ...
    """
    print("=" * 55)
    print("  XGBOOST PIPELINE  (80/20 split, recursive)")
    print("=" * 55)
...
if __name__ == "__main__":
    ...
    # Hub 1
    forecast_df_hub1, best_params_hub1, metrics_hub1 = xgboost_pipeline(
        df_hub1,
        max_period=50,
        train_ratio=0.8,
        postfix="hub1",
    ...
    # Hub 2
    forecast_df_hub2, best_params_hub2, metrics_hub2 = xgboost_pipeline(
        df_hub2,
        max_period=50,
        train_ratio=0.8,
        postfix="hub2",
    ...
    out_path = Path(__file__).resolve().parent.parent / "data" / "forecast_xgboost.csv"
