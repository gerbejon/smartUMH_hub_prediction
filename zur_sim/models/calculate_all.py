        ...
        if folder.endswith("2026-02"):
            ...
            for col in df.columns:
                ...
                if not col in result_dict:
                    result_dict[col] = {}
                    # SARIMA
                    forecast_df, best_params, metrics = main_sarima(ts)
                    result_dict[col]['sarima'] =  metrics

                    # ETS
                    forecast_df, best_params, metrics = main_ets(ts)
                    result_dict[col]['ets'] = metrics

                    # PROPHET
                    forecast_df, best_params, metrics = main_prophet(ts)
                    result_dict[col]['prophet'] = metrics

                    # NHITS
                    forecast_df, best_params, metrics = main_nhits(ts)
